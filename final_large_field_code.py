...
def get_rainfall_from_excel():
    # Load the Excel file
    df = pd.read_excel("E:\\vscode\\python\\weather_data.xlsx", header=None)  # No headers since you're accessing specific cells
        ...
        if soil_moisture is not None:
            ...
            if motor_state != last_motor_state:
                if motor_state:  # If we are turning the motor ON
                    ...
                    log_motor_status(False, soil_moisture, temperature, humidity, rainfall, 0)  # Run time is 0 when turned off
